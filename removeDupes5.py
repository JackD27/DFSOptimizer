...
DKdata = pd.read_csv('DKSalariesClassicWNBA.csv')
playerData = pd.read_csv('WNBAplayerStats.csv')
sport = 0
if sport == 0:
  DKdata.drop_duplicates(subset='Name', inplace=True)
  DKData = DKdata[['Name', 'TeamAbbrev', 'AvgPointsPerGame', 'Position', 'Roster Position', 'Salary', 'ID', 'Game Info']]
  ...
  newData = newData[['Name', 'TeamAbbrev', 'AvgPointsPerGame', 'MPG', 'DKPPM', 'ProjMin', 'DKProjFPTs','jackAPPG', 'jackPPM', 'jackProjFPTs', 'Position', 'Roster Position', 'Salary', 'ID', 'Game Info']]
else:
  print('yes')

DKdata.drop_duplicates(subset='Name', inplace=True)
CFLData = newData[['Name','Position', 'Roster Position','Salary', 'TeamAbbrev', 'AvgPointsPerGame', 'ID', 'Game Info', 'MPG', 'DKPPM', 'ProjMin', 'DKProjFPTs','jackAPPG', 'jackPPM', 'jackProjFPTs']]
...
CFLData = CFLData[['Name','Position', 'Roster Position', 'ID', 'TeamAbbrev','Game Info', 'Salary', 'AvgPointsPerGame', 'MPG', 'DKPPM', 'ProjMin', 'DKProjFPTs','jackAPPG', 'jackPPM', 'jackProjFPTs']]
...
fileName = "SaveThisTo_myOwnData5"
